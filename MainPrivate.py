...
class Player(object):
    ...
    def finalize_results(self, data):
        ...
        if self.advanced_results:
            ...
            max_titans = np.maximum(1, 
                (10 + np.minimum(np.floor(stage.number/1000)*4, 20))).astype(np.int)
            for i in range(20):
                self.splash_array[damage_splashed>(i+1)*stage.titan_hp] = (i+1)
                self.splash_array = np.minimum(self.splash_array, max_titans)
                # Splash penalty is just for model testing
                self.splash_array_penalty[damage_splashed>(i+1)*(2**i)*stage.titan_hp] = (i+1)
                self.splash_array_penalty = np.minimum(self.splash_array_penalty, max_titans)
...
if __name__ == '__main__':
    ...
    plot_tap_damage(player, data.stage)
